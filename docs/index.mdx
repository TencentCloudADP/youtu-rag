...
Alternatively, you may consider deploying the backend services on TencentCloud Ti-one platform. See [Deploying with TencentCloud Ti-one](/docs/ti-one) for more details.

## Step 1: Deploy Youtu Embedding Service
Follow the instructions in [Deploying Youtu Embedding Locally](/docs/youtu-embedding/deploying_locally) or [Deploying Youtu Embedding with Docker](/docs/youtu-embedding/docker_image) to set up the Youtu Embedding service. Then add the following configuration to your `.env` file:
...
## Step 2: Deploy HiChunk Service
Follow the instructions in [Deploying HiChunk Locally](/docs/hichunk/deploying_locally) or [Deploying HiChunk with Docker](/docs/hichunk/docker_image) to set up the HiChunk service. Then add the following configuration to your `.env` file:
...
Follow the instructions in [Deploying Youtu Parsing Locally](/docs/youtu-parsing/deploying_locally) or [Deploying Youtu Parsing with Docker](/docs/youtu-parsing/docker_image) to set up the Youtu Parsing service. Then add the following configuration to your `.env` file:
