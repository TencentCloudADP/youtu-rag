...
本文档提供使用 vllm 部署 Youtu HiChunk 作为 Youtu-RAG 后端服务的说明，提供分层分块功能。
...
从[官方仓库](https://huggingface.co/tencent/Youtu-HiChunk)下载预训练模型权重：
...
安装使用 vllm 服务 Youtu HiChunk 所需的依赖项
...
在 vllm 安装目录的 `vllm/model_executor/models/` 中创建名为 `utu_v1.py` 的文件（如果不存在），内容如下：
...
在 `vllm/model_executor/models/` 目录中创建或替换名为 `registry.py` 的文件，内容如下：
...
# 使用 vLLM 服务 Youtu HiChunk

运行以下脚本在端口 `8501` 上使用 vLLM 服务 Youtu HiChunk 模型：
...
运行后，Youtu HiChunk 通过 OpenAI 兼容的 API 提供服务。可用的端点如下：

| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `/v1/completions` | POST | 生成文本补全 |
| `/v1/chat/completions` | POST | 生成聊天补全 |
| `/health` | GET | 健康检查端点 |
